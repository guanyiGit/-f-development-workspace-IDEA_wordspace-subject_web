<template>
    ...
    <div class="centoer">
        ...
        <div class="btns">
            <el-button class="fenzhi" type="primary" size="mini" v-on:click="bachSerScore">分值批量设置</el-button>
        ...
        <div class="selecteds">
            ...
            <span class="fenzhi" style="margin-left: 20px">计算总分： </span>
            <span class="fenzhi" style="color: red">{{countScore}}分</span>
            <span class="fenzhi" style="margin-left: 20px">试卷设置的总分：</span>
            <span class="fenzhi" style="color: red">{{parmQuestion.paperTotalScore}}分</span>
        ...
        <div class="tablPerp">
            <table class="mtable">
                <tr>
                    ...
                    <td>试题名称</td>
                    <td>难度</td>
                    <td v-if="parmQuestion.paperType!=0">分值</td>
                    <td>操作</td>
                </tr>
                <tr v-for="(item,index) in tableData" :key="index">
                    ...
                    <td v-if="parmQuestion.paperType!=0">
                        <input :id="index" class="el-input__inner" autocomplete="off" type="text" :value="item.score"  style="width: 80px;vertical-align:middle;" v-on:change="changeFen(item.subjectId,index)"/>
                    </td>
            ...
            <div class="tan3 bgw rel" style="height: 350px">
                ...
                <div style="margin-left: 15%;margin-top: 5%">
                    ...
                    <div style="margin-top: 5%">
                        ...
                        <span>&nbsp;分</span>
                        <p style="color: red;margin-top: 20px">注意：分值只能输入最多带1位小数的正实数！</p>
                    </div>
                </div>
    ...
    export default {
        ...
        methods:{
            ...
            changeFen:function (subjectId,index) {
                var fenfen=document.getElementById(index).value;
                if(!(/^(0|[1-9][0-9]*)+(.[0-9]{1})?$/.test(fenfen))){
                    this.$alert("分值只能输入最多带1位小数的正实数！")
                    return
                }
                fenfen=parseFloat(fenfen);
                this.countScore=0
            ...
            complete:function () {//完成
                if (this.parmQuestion.paperType==0) {
                    ...
                    for (var i=0;i<this.parmQuestion.tSubjectRefPapers.length;i++) {
                        this.parmQuestion.tSubjectRefPapers[i].score=null
                    }
                }
                if (this.parmQuestion.judge==1 && this.parmQuestion.paperId!=null) {
                    ...
                    this.parmQuestion.paperCreater=getCookie("userId");
                    if (this.parmQuestion.paperType!=0) {
                        if (this.countScore != parseInt(this.parmQuestion.paperTotalScore)) {
                            this.$alert("计算总分与设置的总分不相等，请调节试题分值！")
                            return
            ...
            tijiao:function(){
                if(!(/^(0|[1-9][0-9]*)+(.[0-9]{1})?$/.test($("#mtfz").val().trim()))){
                    $("#mtfz").val(0)
                    // this.$alert("试题分数需填正整数数字！")
                    return;
                }
                this.countScore=0;
                for (var i=0;i<this.tableData.length;i++){
                    if (this.tableData[i].isCheckd==true) {
                        this.tableData[i].score=$("#mtfz").val().trim();
